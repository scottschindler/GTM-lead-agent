...
If `pipeline_writer` fails, returns invalid structured output, returns the wrong number of hypotheses/opportunities, or omits required fields, log that fast mode fell back in your narration and use the older stage-by-stage skill path: `load_skill` `qualification`, `hypothesis`, `opportunity-mapping`, `content-generation`, and `sequence-planning` as needed. The fallback path must still produce exactly 2 hypotheses, exactly 2 opportunities, at most 2 customer stories, at most 2 objection responses, a 120-word email, and no learning step.
